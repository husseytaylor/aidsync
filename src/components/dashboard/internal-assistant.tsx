...
const RESPONSE_WEBHOOK_URL = 'https://bridgeboost.app.n8n.cloud/webhook/5636b979-4a64-4db4-910d-d1bd780e6c3d';
const FIRST_ASSISTANT_PROMPT = "Hi! I’m AidSync’s internal assistant. I can help you analyze chat and voice interaction data—what would you like to look into?";
...
export function InternalAssistant() {
  ...
  useEffect(() => {
    setIsMounted(true);
    const storedSessionId = localStorage.getItem('internal_chat_session_id');
    if (storedSessionId) {
  ...
  const handleOpen = useCallback(() => {
    if (messages.length === 0) {
      let currentSessionId = localStorage.getItem('internal_chat_session_id');
      if (!currentSessionId) {
        currentSessionId = crypto.randomUUID();
        setSessionId(currentSessionId);
        localStorage.setItem('internal_chat_session_id', currentSessionId);
      } else {
        setSessionId(currentSessionId);
      }
      setMessages([
    ...
    <>
      <div className={cn(
        ...
        <motion.button
          ...
          aria-label="Open Internal AI Assistant"
        >
        ...
        <div className="h-[70vh] flex flex-col rounded-2xl border-accent/20 bg-black/35 backdrop-blur-xl shadow-[0_8px_24px_rgba(72,209,204,0.25)] overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between border-b border-white/10">
            <div className="flex items-center gap-3">
              ...
              <CardTitle className="font-headline text-lg text-secondary-foreground">Internal AI Assistant</CardTitle>
            </div>
          ...
          <CardFooter className="border-t pt-4 border-white/10">
            <form onSubmit={handleSubmit} className="relative flex w-full items-center">
              <Input
                ...
                placeholder="Analyze interaction data..."
                disabled={isPending}
                autoFocus
